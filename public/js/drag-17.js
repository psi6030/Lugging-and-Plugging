...
// add actual inlets and outlets to boxes. make sure there's a free
// outlet we can still drag but connected outlets shouldn't be
// draggable. this is done by re-rendering box.el after something gets
// changed on the box
  ...
  this.el = $('<div class="box" />')
    ...
    .draggable({ 
      ...
      drag: function ( event, ui ) {
        self.x = ui.helper.offset().left - self.opts.panel.offset().left;
        self.y = ui.helper.offset().top - self.opts.panel.offset().top;
    ...
    $('<li class="outlet" />')
      ...
      .draggable({ 
        ...
        drag: function ( event, ui ) {
          ...
          ui.helper.path.cable = self.opts.boxes.paper.path( s ).attr({'stroke':'#888','stroke-width':4});
        },
        stop: function ( event, ui ) {
           if ( ui.helper.path.cable ) {
             ui.helper.path.cable.remove();
